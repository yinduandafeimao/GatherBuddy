...
public partial class Interface
{
    ...
    private class AlarmCache
    {
        ...
        public sealed class AlarmSelector : ItemSelector<AlarmGroup>
        {
            ...
            protected override void OnDrop(object? data, int idx)
            {
                ...
                var group = Plugin.AlarmManager.Alarms[idx];
                Plugin.AlarmManager.DeleteAlarm(obj.Group, obj.AlarmIdx);
                Plugin.AlarmManager.AddAlarm(group, obj.Alarm);
            }
        ...
        public static readonly string SoundIdNames =
            string.Join("\0", SoundIds.Select(s => s == Sounds.None ? "无声音" : $"Sound {s.ToIdx()}"));
    ...
    private void DrawAlarmInfo(ref int alarmIdx, AlarmGroup group)
    {
        ...
        if (ImGuiUtil.DrawDisabledButton(FontAwesomeIcon.Trash.ToIconString(), IconButtonSize, "删除该闹钟...", false, true))
            Plugin.AlarmManager.DeleteAlarm(group, alarmIdx--);
        ImGui.TableNextColumn();
        ImGui.SetNextItemWidth(SetInputWidth);
        var name = alarm.Name;
        if (ImGui.InputTextWithHint("##name", CheckUnnamed(string.Empty), ref name, 64))
            Plugin.AlarmManager.ChangeAlarmName(group, alarmIdx, name);
        ImGuiUtil.HoverTooltip("名称是可选的，用于输出到聊天框中的警报消息");

        ImGui.TableNextColumn();
        if (ImGui.Checkbox("##Enabled", ref enabled) && enabled != alarm.Enabled)
            Plugin.AlarmManager.ToggleAlarm(group, alarmIdx);
        ImGuiUtil.HoverTooltip("启用这个闹钟");

        ImGui.TableNextColumn();
        if (_alarmCache.ItemCombo.Draw(alarm.Item.InternalLocationId - 1, out var newIdx))
            Plugin.AlarmManager.ChangeAlarmItem(group, alarmIdx, GatherBuddy.UptimeManager.TimedGatherables[newIdx]);
        ...
        _alarmCache.Selector.CreateDropTarget<AlarmWindowDragDropData>(d => Plugin.AlarmManager.MoveAlarm(group, d.AlarmIdx, localIdx));
        ...
        if (ImGui.IsItemDeactivated())
            Plugin.AlarmManager.ChangeAlarmOffset(group, alarmIdx, Math.Clamp(_alarmCache.ChangedSecondOffset, 0, RealTime.SecondsPerDay));
        ImGuiUtil.HoverTooltip("提前触发闹钟的秒数\n--可拖动--");

        ImGui.TableNextColumn();
        var printMessage = alarm.PrintMessage;
        if (ImGui.Checkbox("##PrintMessage", ref printMessage))
            Plugin.AlarmManager.ChangeAlarmMessage(group, alarmIdx, printMessage);
        ImGuiUtil.HoverTooltip("当此闹钟触发时，输出消息到聊天框");
        ...
        if (ImGui.Combo("##Sound", ref idx, AlarmCache.SoundIdNames))
            Plugin.AlarmManager.ChangeAlarmSound(group, alarmIdx, AlarmCache.SoundIds[idx]);
        ImGuiUtil.HoverTooltip("当此闹钟触发时，播放此声效");

        ImGui.TableNextColumn();
        if (DrawLocationInput(alarm.Item, alarm.PreferLocation, out var newLocation))
            Plugin.AlarmManager.ChangeAlarmLocation(group, alarmIdx, newLocation);
        ...
        if (time.Start > now)
            ...
            ImGuiUtil.DrawTextButton("当前可收集", size, ColorId.ChangedLocationBg.Value());
    }
    ...
    private void DrawGroupData(AlarmGroup group, int idx)
    {
        if (ImGuiUtil.DrawEditButtonText(0, _alarmCache.EditGroupName ? group.Name : CheckUnnamed(group.Name), out var newName,
            ...
            Plugin.AlarmManager.ChangeGroupName(idx, newName);
        ...
        if (ImGuiUtil.DrawEditButtonText(1, _alarmCache.EditGroupDesc ? group.Description : CheckUndescribed(group.Description),
            ...
            Plugin.AlarmManager.ChangeGroupDescription(idx, newDesc);
        var enabled = group.Enabled;
        if (ImGui.Checkbox("启用", ref enabled) && enabled != group.Enabled)
            Plugin.AlarmManager.ToggleGroup(idx);
        ImGuiUtil.HoverTooltip(
            "启用该闹钟组。只有该组中已启用的闹钟才会被列为为活动状态");
    }
    ...
    private void DrawToggleAll(AlarmGroup group)
    {
        ...
        ImGuiUtil.HoverTooltip("启用所有已禁用的闹钟，或禁用所有闹钟");
        ...
        for (var i = 0; i < group.Alarms.Count; ++i)
        {
            if (group.Alarms[i].Enabled != allEnabled)
                Plugin.AlarmManager.ToggleAlarm(@group, i);
        }
    }
    ...
    private void DrawAlarmTable(AlarmGroup group, int idx)
    {
        ...
        ImGui.TableNextColumn();
        if (ImGuiUtil.DrawDisabledButton(FontAwesomeIcon.Plus.ToIconString(), IconButtonSize, "添加新的闹钟...", false, true))
            Plugin.AlarmManager.AddAlarm(group, _alarmCache.CreateAlarm());
        ImGui.TableNextColumn();
        ImGui.SetNextItemWidth(SetInputWidth);
    ...
    {
        if (ImGuiUtil.DrawDisabledButton(FontAwesomeIcon.Copy.ToIconString(), IconButtonSize, "复制当前闹钟组到剪贴板",
        ...
        if (ImGuiUtil.DrawDisabledButton("添加悬浮窗预设。", Vector2.Zero, "通过该闹钟组创建一个新的采集悬浮窗预设",
        ...
        ImGuiComponents.HelpMarker("使用 /gather alarm 命令来收集当前触发闹钟的物品。\n"
          + "使用 /gatherfish alarm 命令来收集当前触发闹钟的鱼类。");
    }
    ...
    private void DrawAlarmTab()
    {
        ...
        ImGuiUtil.HoverTooltip("你是否经常会错过重要时间？\n"
          + "这里可以设置你自己的闹钟！");
        if (!tab)
            return;
